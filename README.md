...
```
next-k-frontend/fleet.html ──/grid-bot/*──► Next K Protocol (+ Node 网格)
next-k-api (OI/ORB 等) ──/api/binance/*──► Next K Protocol (币安执行)
```

网格**不经** next-k-api。看板直连本服务 `/grid-bot`。详见 `docs/GRID.md`。
