  ...
  const cards = useMemo(() => {
    ...
    const items = [
      { t: t('fp.funding'), val: btcFunding, fmt: (x) => `${(x * 100).toFixed(4)}%`,
        color: btcFunding > 0.0003 ? '#C24B45' : btcFunding < -0.0001 ? '#2F8A52' : undefined,
        tip: t('fp.fundingTip') },
      ...
      { t: t('fp.oi'), val: btcOi, fmt: (x) => `${x >= 0 ? '+' : ''}${x.toFixed(2)}%`,
        color: btcOi > 0 ? '#2F8A52' : '#C24B45', tip: t('fp.oiTip') },
      ...
      { t: t('fp.cbprem'), val: cbPrem, fmt: (x) => `${Number(x).toFixed(4)}`,
        color: cbPrem > 0 ? '#2F8A52' : '#C24B45', tip: t('fp.cbpremTip') },
      ...
      { t: t('fp.etf'), val: etf, fmt: (x) => `${x >= 0 ? '+' : ''}${(x / 1e6).toFixed(0)}M`,
        color: etf >= 0 ? '#2F8A52' : '#C24B45', tip: t('fp.etfTip') },
    ]
