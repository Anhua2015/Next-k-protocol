 ...
 */
export function buildGrid({ lower, upper, gridCount, priceStep }) {
  if (!(upper > lower)) throw new Error('upper 必须大于 lower');
  if (!(gridCount >= 2)) throw new Error('gridCount 至少为 2');
  let spacing = (upper - lower) / gridCount;
  const levels = [];
  for (let i = 0; i <= gridCount; i++) {
    let lvl = lower + i * spacing;
    if (priceStep) lvl = snapLevel(lvl, priceStep);
    levels.push(round(lvl));
  }
  if (levels.length > 1) spacing = levels[1] - levels[0];
  return { levels, spacing: round(spacing), count: gridCount };
}

function snapLevel(px, step) {
