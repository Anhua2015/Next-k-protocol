...
process.on('uncaughtException', (e) => console.error('[BitgetGrid] uncaughtException:', e?.message ?? e));
process.on('unhandledRejection', (e) => console.error('[BitgetGrid] unhandledRejection:', e?.message ?? e));
...
server.listen(cfg.port, async () => {
  console.log('\n网格机器人已启动 [实盘 LIVE]（extended-grid 源码 · Bitget 适配）');
  console.log(`仪表盘: http://localhost:${cfg.port}`);
  console.log(`行情数据源: 实时 Bitget ${exchange.apiUrl || ''}`);
  console.log('⚠️ 实盘模式：将使用真实资金在 Bitget 主网下单（策略语义与上游一致：净仓/单向）。');
  if (cfg.authToken) console.log('🔒 看板/API 已启用 GRID_AUTH_TOKEN 保护。\n');
  else console.log('');
