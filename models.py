...
class StatusOut(BaseModel):
    ...
    max_entry_spread_bps: float = Field(..., description="最大允许盘口价差 bps，0 表示不启用")
    market_entry_fallback: bool = Field(..., description="LIMIT_FOK 失败后是否回退 MARKET")
    pnl_auto_sync_enabled: bool = Field(..., description="PnL 自动同步是否启用")
    pnl_last_sync_at: Optional[str] = Field(None, description="PnL 最近同步时间")
